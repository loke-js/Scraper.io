...
import { CoinsIcon, CreditCard, IndianRupee, IndianRupeeIcon } from 'lucide-react'
                ...
                {CreditsPack.map((pack)=>(
                    <div key={pack.id} className='flex items-center space-x-3 bg-secondary/50 rounded-lg p-3 hover:bg-secondary'>
                        ...
                        <Label htmlFor={pack.id}  className='flex justify-between w-full cursor-pointer'>
                            ...
                            <span className='font-bold text-primary flex items-center gap-1'><IndianRupee size={12}/>{(pack.price/100).toFixed(2)}</span>
                        </Label>
                    </div>
                ))}
                </RadioGroup>
